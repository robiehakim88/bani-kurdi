...
class User extends Authenticatable
{
    ...
    {
        // aslinya ini return $this->belongsTo(User::class);
    ...
    // app/User.php
    //perubahan function diatas
    //public function profileLink($type = 'profile', $raw = false)
    //{
        // Tentukan route berdasarkan type ('profile' atau 'chart')
    //    $route = ($type == 'chart') ? 'users.chart' : 'users.show';
    
        // Jika $raw bernilai true, kembalikan hanya nama tanpa link
    //    if ($raw) {
    //        return $this->name;
     //   }
    
        // Kembalikan link HTML jika $raw bernilai false
       // return '<a href="' . route($route, $this->id) . '">' . $this->name . '</a>';
    //}
    ...
    // Tambahkan method untuk menentukan hubungan generasi
    public function getGenerationalRelationshipAttribute()
    {
        // Cari root ancestor (pendiri keluarga)
        $rootAncestor = $this->getRootAncestor();
    
        // Hitung kedalaman generasi dari root
        $generationDepth = $this->getGenerationDepthFrom($rootAncestor);
    
        // Tentukan label berdasarkan kedalaman
        return $this->getGenerationLabel($generationDepth);
    }
    
    private function getRootAncestor()
    {
        $current = $this;
    ...
    private function getGenerationDepthFrom(User $ancestor)
    {
        $depth = 0;
        $current = $this;
    ...
    private function getGenerationLabel(int $depth)
    {
        switch ($depth) {
            case 0: return 'Pendiri Keluarga';
            case 1: return 'Anak';
            case 2: return 'Cucu';
            case 3: return 'Cicit';
            case 4: return 'Buyut';
            case 5: return 'Canggah';
            default: return 'Generasi ke-' . ($depth + 1);
        }
    }
