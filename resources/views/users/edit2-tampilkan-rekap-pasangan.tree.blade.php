...
<div id="wrapper">
    <span class="label">
        <span class="{{ isDeceased($user) ? 'deceased' : '' }}">
            ...
            @if(isDeceased($user))
                <i class="far fa-times-circle" style="color: #8B0000; font-size: 0.7em; margin-left: 3px;" title="Sampun sedo"></i>
            @endif
        </span>
        <br>
        @if($user->gender_id == 1)
...
<div class="container">
    ...
    <div class="row">
        @foreach($levels as $label => $data)
            @if($data['total'] > 0)
                <div class="col-md-2">
                    <div class="text-center">{{ ucfirst(str_replace('_', ' ', $label)) }}</div>
                    <div class="text-left">
                        <strong>{{ $data['alive'] }}</strong> sugeng /
                        <strong>{{ $data['deceased'] }}</strong> sedo
                        @if($data['spouses_alive'] + $data['spouses_deceased'] > 0)
                            <br>
                            Menantu:
                            <strong>{{ $data['spouses_alive'] }}</strong> sugeng /
                            <strong>{{ $data['spouses_deceased'] }}</strong> sedo
                        @endif
                    </div>
                </div>
            @endif
